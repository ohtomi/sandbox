...
public class Main {
	...
	private void basicUsage() throws Exception {
		...
		PojoContainer orig = new PojoContainer(1, "ohtomi", new PojoItem(11,
				"kenichi"));
		byte[] bytes = objectMapper.writeValueAsBytes(orig);
		PojoContainer value = objectMapper
				.readValue(bytes, PojoContainer.class);
	...
	private void request() throws Exception {
		...
		req.setId(2);
		req.setMethod("foo");
		req.setArgs(new Object[] { 1, 2.0,
				new PojoContainer(1, "ohtomi", new PojoItem(11, "kenichi")) });
		...
		PojoContainer pojo = objectMapper.convertValue(args[2],
				PojoContainer.class);
	...
	private void response() throws Exception {
		...
		res.setType(1);
		res.setId(2);
		res.setError(new UserException("timeout", "3ms", new PojoItem(11,
				"kenichi")));
		res.setResult(new PojoContainer(1, "ohtomi",
				new PojoItem(11, "kenichi")));
		...
		// ObjectMapper cannot convert error with class of RemoteException, it
		// needs the class of concrete java type.
		UserException ex = objectMapper
				.convertValue(error, UserException.class);

		Object result = res2.getResult();
		// ObjectMapper can convert result with class of PojoContainer, which
		// has some user-defined java type.
		PojoContainer pojo = objectMapper.convertValue(result,
				PojoContainer.class);
